...
### 4. 대규모 연결 시 Network I/O 및 소켓 오버헤드

* 시뮬레이터(Job) 수만큼 물리적인 Connection을 생성할 경우, Handshake 비용과 File Descriptor 고갈로 인한 시스템 한계 직면
* 무의미한 네트워크 연결 유지 비용으로 인해 시뮬레이션 엔진 본연의 성능 저하 우려

👉 **해결**: Connection Multiplexing (연결 재사용 및 다중화) 설계 적용
* 물리적인 단일 Connection(Session) 위에서 다수의 시뮬레이터 스트림 데이터를 논리적으로 다중화(Multiplexing)하여 전송.
* 불필요한 TCP/Socket 오버헤드를 제거하여, CPU 컨텍스트 스위칭 및 메모리 점유율을 최적화.
...
- 물리적 단일 Connection을 통한 데이터 다중화(Multiplexing) 전송으로 불필요한 소켓 오버헤드를 최적화
